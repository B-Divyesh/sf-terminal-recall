...
| `--teal` | `#006c67` | links, match marks, success; 5.4:1 on paper |
...
`public/terminal-recall-share.webp` is a 1200×630 crop composed from that original
collage with ImageMagick. `public/apple-touch-icon.png` is a 180×180 crop from the
same source. `public/terminal-recall-demo.cast` is not illustrative artwork: it is
an asciinema v2 transcript generated by `scripts/generate-recording.mjs` from the
compiled v0.1.4 CLI and the repository's bundled deploy sample.
