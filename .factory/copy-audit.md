...
Audited 2026-08-28. The landing and demo copy contain no banned marketing words.
Every sentence is at most 22 words. Commands and navigation labels are excluded
...
| Search the sample deploy record | 5 | Pass |
| This example uses separate demo data and never changes your saved records. | 11 | Pass |

## First-screen read

The headline names the job in six words. The next sentence identifies developers
and the lost-session situation. The adjacent action and outcome explain the
first click. All three facts remain visible at 390×844.

## Terminology

| Concept | One product word |
| --- | --- |
| Saved command output | record |
| Intentionally saving output | capture |
| Shareable text file | excerpt |
| Pattern-based secret removal | redaction |
| Isolated browser example | demo |
